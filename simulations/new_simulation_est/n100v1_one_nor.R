...
for (ss in 1:iter)
{
  ...
  mean_node_1<-cbind(mu1,mu2,mu3,mu4,mu5+mu9*x_cov,mu6
                     ,mu7+mu11*x_cov,mu8,mu9,mu10,mu11,matrix(0,nrow=length(mu1),ncol=(pn-11))+0.01*t_true)[c(1,diff(t,lag=1))>=0.0001,]
  ...
  plot(y=mean_node_1[,5],x=t)
  lines(data_fitted[,5]+x_cov*data_fitted_cov[,5],x=t)
  ...
  X_big_int_exp_intcep_1<-cbind(1,t,X_big_int,x_cov*X_big_int_cov,t*x_cov,x_cov)
